...
class halaman_list {
	...
	public function halamanList($totalData, $numToDisplay, $currentPage, $modul)
	{
		...
		{
			$keluaran = $keluaran."<a title=\"Klik untuk langsung ke halaman ... \" onclick=\"nextTo('$modul','$totalPages'); return false;\" href=\"#pageCursor\">Halaman $currentPage dari $totalPages</a> &bull; ";			
		}
		if($currentPage > 1)
			{
				$a = $currentPage -1;
				$keluaran =  $keluaran . "<a href=\"#pageCursor\" onClick=\"javascript:nextPage('$modul','$a');\">&laquo; sebelumnya</a>";
		...
		if($totalPages <= $indexEndPage)
		{
			...
			for($a=$indexStartPage; $a <= $totalPages; $a ++)
			{
				...
				{
					$keluaran =  $keluaran . "<a href=\"#pageCursor\"  onClick=\"javascript:nextPage('$modul','$a');\">&nbsp;$a&nbsp;</a>";
				}
			}
			if($currentPage ==$totalPages){
				...
				$a = $a -1;
				$keluaran =  $keluaran . " <a href=\"#pageCursor\" onClick=\"javascript:nextPage('$modul','$a');\">berikutnya &raquo;</a>";
			}
		}
		else
		{
	...
	public function halamanListPopup($totalData, $numToDisplay, $currentPage, $modul)
	{
		$totalPages = $this->pager($totalData, $numToDisplay);
		$totalPerPages = 5;
		$totalGroup = $totalPages / $totalPerPages ;
		...
		{
			$keluaran = $keluaran."<a title=\"Klik untuk langsung ke halaman ... \" onclick=\"nextToPopup('$modul','$totalPages'); return false;\" href=\"#pageCursor\">Halaman $currentPage dari $totalPages</a> &bull; ";			
		}
		if($currentPage > 1)
			{
				$a = $currentPage -1;
				$keluaran =  $keluaran . "<a href=\"#pageCursor\" onClick=\"javascript:nextPagePopup('$modul','$a');\">&laquo; sebelumnya</a>";
		...
		if($totalPages <= $indexEndPage)
		{
			...
			for($a=$indexStartPage; $a <= $totalPages; $a ++)
			{
				...
				{
					$keluaran =  $keluaran . "<a href=\"#pageCursor\"  onClick=\"javascript:nextPagePopup('$modul','$a');\">&nbsp;$a&nbsp;</a>";
				}
			}
			if($currentPage ==$totalPages){
				$keluaran =  $keluaran . "<span class=\"disabled\">berikutnya &raquo;</span>  ";
				...
				$a = $a -1;
				$keluaran =  $keluaran . " <a href=\"#pageCursor\" onClick=\"javascript:nextPagePopup('$modul','$a');\">berikutnya &raquo;</a>";
			}
		}
		else
		{
...
function nextPagePopup(url,currentPage){
...
function nextToPopup(url,totalPages){
	var jump_page = window.prompt("Masukkan no halaman yang ingin Anda tuju:");
	totalPages = totalPages*1;
	var currentPage = jump_page*1;
