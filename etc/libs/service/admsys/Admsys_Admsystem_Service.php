<?php
class Admsys_Admsystem_Service {
	...
	public function getUserAdmlainExist($i_bumn)
	{
		...
		try {
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
			$result = $db->fetchAll("SELECT distinct i_ssouser_userid, i_bumn, i_ssouser_email, i_ssouser_telp 
									from a_sso_user_0_tr 
									where i_bumn = '$i_bumn' 
									and (i_ssouser_appl != 'admutama' and i_ssouser_appl != 'admsys')
									and (i_ssouser_role != 'admutama' and i_ssouser_role != 'admsys')");	
	...
	public function getAplikasi() {
	   ...
	   try {
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
			 ...
			 $result = $db->fetchAll("select distinct i_ssouser_appl  from a_sso_user_admin_tr 
										where i_ssouser_appl != 'admsys' and 
											  i_ssouser_appl != 'admutama'");
	 
	 //print_r($result);
			$jmlresult = count($result);
			for ($j = 0; $j < $jmlresult; $j++) 
			{
				$hasilAkhir[$j] = array("i_ssouser_appl"  =>(string)$result[$j]->i_ssouser_appl);
			}	
		     return $hasilAkhir;
			 unset($hasilAkhir);
	   } catch (Exception $e) {
	...
	public function getAplikasiRole() {
	   $registry = Zend_Registry::getInstance();
	   $db = $registry->get('db');
	   ...
	   try {
		 $db->setFetchMode(Zend_Db::FETCH_OBJ); 
			 ...
			 $result = $db->fetchAll("select i_ssouser_appl,i_ssouser_role, e_ssouser_desc  from a_sso_user_admin_tr 
										where i_ssouser_appl != 'admsys' and 
											  i_ssouser_appl != 'admutama'");
	 ...
	 //print_r($result);
			...
			for ($j = 0; $j < $jmlresult; $j++) 
			{
				$hasilAkhir[$j] = array("i_ssouser_appl"  =>(string)$result[$j]->i_ssouser_appl,
										...
										"e_ssouser_desc"  =>(string)$result[$j]->e_ssouser_desc);
			}	
		     return $hasilAkhir;
			 unset($hasilAkhir);
	   } catch (Exception $e) {
