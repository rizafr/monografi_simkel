...
class Otoritas_Service {
	...
	public function getOtoritasList() {
	...
	public function cariOtoritasList(array $dataMasukan, $pageNumber, $itemPerPage,$total) {
		...
		if($kategoriCari == "") { $kategoriCari ="nama";}		
			...
			for ($j = 0; $j < $jmlResult; $j++) {
				...
				$n_otoritas= $db->fetchOne("SELECT nama FROM tr_otoritas_user WHERE id = '$apl' ");	
	...
	public function otoritasInsert(array $dataMasukan) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		try {
			$db->beginTransaction();
	...
	public function detailOtoritasById($id) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		try {
			$db->setFetchMode(Zend_Db::FETCH_OBJ); 
	...
	public function otoritasUpdate(array $dataMasukan) { 
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		try {
			$db->beginTransaction();
	...
	public function otoritasHapus(array $dataMasukan) {
		$registry = Zend_Registry::getInstance();
		$db = $registry->get('db');
		try {
			$db->beginTransaction();
