...
function writedate(theday) {
  ...
  d=mod(d,7)+1;
  var prout="<center><table border=0 cellpadding=0 cellspacing=2 width=160 class='cld'><tr><td colspan=8 class='bln'>";
  prout+="<table width=100% border=0 cellpadding=0 cellspacing=>";
  prout+="<tr><td width=10 align='left'><img onclick=\"writedate('"+dprev+"')\" src=\"/images/dprev.gif\" class=\"klik\" border=0></td>";
  prout+="<td  align='center' class='boxedbln'>"+amon[month]+" "+year+"</td>";
  prout+="<td width=10 align:'right'><img onclick=\"writedate('"+dnext+"')\" src=\"/images/dnext.gif\" class=\"klik\" border=0></td></tr>";
  prout+="</table>";
  prout+="</td></tr><tr>";
  for (c=1;c<=7;c++) {
    if (c==1) prout+="<td class=\"boxedh md\">"+aday[c]+"</td>";
  ...
  for (c=1; c<=maxday; c++) {
    rundate=c+moyear;
    if (c==1) {
      for(cc=1;cc<d;cc++) prout+="<td class=\"boxedx\">&nbsp;</td>";
      if (p==1) {
        if (datedarray.in_array(rundate)) dclass="boxedmo";
        else dclass="boxed md";
      }
      else {
    ...
    var nstyle=" ";
    if (today==rundate) nstyle="default";
    ...
    else prout+="<td id=\""+c+"\" class=\""+dclass+" "+nstyle+"\">"+c+"</td>";
    if (mod(p,7)==0) {
      prout+="</tr><tr>";
      p=1;
    }
